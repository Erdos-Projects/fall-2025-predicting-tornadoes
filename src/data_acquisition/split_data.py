...
for station in stations:
    ...
    station_path =project_root/pathlib.Path('Data/stations/raw')/pathlib.Path(f'Station_{station}.csv')
    station_csv = station_df.to_csv(station_path,index = False)
    ...
    if memory_mb >=100:
        ...
        i=1 # Counter
        for split in splits:
            split_path = project_root/pathlib.Path('Data/stations/raw')/pathlib.Path(f'Station_{station}_Part_{i}.csv')
