...
## **2. Data Overview**
- [ ] Record `df.shape` (rows × columns).
- [ ] Use `df.info()` to inspect dtypes and non-null counts.
- [ ] Use `df.describe()` for numeric summaries.
...
- [ ] Check for placeholder codes (e.g., 9999, -9999). See ISD documentation.
...
## **4. Type Validation**
- [ ] Confirm numeric-like strings were converted properly.
- [ ] Verify categorical columns (e.g., station name, source) remain strings.
- [ ] Ensure dates are `datetime64[ns]` objects.
- [ ] Check for mixed-type columns using `df[col].apply(type).value_counts()`.
